...
use crate::fs::Path;
use crate::state::{CreateFileOpts, CreateFileOptsBuilder, NamespaceId};
...
impl MountInfo {
    ...
    // TODO(new-namespace): these helpers are kept only for the legacy unified
    // client path. They should be removed once client/metanode fetch create
    // policies from NamespaceInfo instead of MountInfo.
...
impl MountOptionsBuilder {
    ...
    pub fn with_conf(_conf: &ClientConf, update: bool) -> Self {
        Self::new().update(update)
    }
    ...
    pub fn namespace_name(mut self, namespace_name: impl Into<String>) -> Self {
        self.namespace_name = Some(namespace_name.into());
        self
    }
