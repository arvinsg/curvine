...
use crate::pd::bgtable::placement::{isolation_score, Labels, PlacementRule};
...
pub struct HashReplicaChecker;

impl HashReplicaChecker {
    fn build_under_replicated_repair(
        bg: &BlockGroupInfo,
        available_replicas: &[u32],
        ctx: &CoordinatorContext,
    ) -> Option<BGOperator> {
        let table = ctx.bgtable_manager.get_table(bg.table_id)?;
    ...
    fn build_over_replicated_repair(
        ...
        let table = ctx.bgtable_manager.get_table(bg.table_id)?;
        let desired = table.replica_count() as usize;
        if candidate_replicas.len() <= desired {
            return None;
        }
        ...
        if to_remove.contains(&bg.primary.node_id) {
            let survivor = bg
                .replica_set
                .iter()
                .copied()
            ...
            match survivor {
                Some(default_target) => {
                    let new_owner =
                        ctx.pick_primary_fallback_excluding(bg, &to_remove, default_target);
                    transfer = Some((bg.primary.node_id, new_owner));
                }
                None => {
                    log::warn!(
                        "BG {} over-replicated: keeping primary {} — no survivor for primary transfer",
                        bg.bg_id,
                        bg.primary.node_id,
                    );
                    to_remove.retain(|&w| w != bg.primary.node_id);
                    if to_remove.is_empty() {
                        return None;
                    }
                }
        ...
        if let Some((from, to)) = transfer {
            builder = builder.transfer_primary(from, to);
        }
    ...
    fn select_replicas_to_remove(
        ...
        let candidates: Vec<u32> = candidate_replicas.iter().copied().collect();
        ...
        let on_blacklist = filter_by_node_state(&candidates, NodeState::Blacklist, ctx);
        let healthy: Vec<u32> = candidates
            .iter()
            .copied()
            .filter(|w| {
                !on_decommission.contains(w) && !on_offline.contains(w) && !on_blacklist.contains(w)
            })
            .collect();
        let healthy_ranked =
    ...
    fn rank_healthy_by_policy(
        ...
        let rule = ctx.bgtable_manager.hash_placement().placement_rule();
        let has_placement = !rule.is_empty() && !rule.location_labels.is_empty();
        let worker_labels = if has_placement {
            ctx.pool_manager.get_workers_labels(candidate_replicas)
        } else {
            HashMap::new()
        };
...
fn repair_available_replicas(bg: &BlockGroupInfo, ctx: &CoordinatorContext) -> Vec<u32> {
    bg.replica_set
        .iter()
        .copied()
        .filter(|&worker_id| is_repair_available(worker_id, ctx))
        .collect()
}

fn is_repair_available(worker_id: u32, ctx: &CoordinatorContext) -> bool {
    matches!(
        ctx.node_manager.get_node(worker_id).map(|node| node.state),
        Some(NodeState::Starting | NodeState::Live | NodeState::Lost)
    )
}

/// Workers in `candidates` whose node currently sits in `state`.
fn filter_by_node_state(
    candidates: &[u32],
    state: NodeState,
    ctx: &CoordinatorContext,
) -> Vec<u32> {
...
impl super::Checker for HashReplicaChecker {
    ...
    fn check_bg(&self, bg: &BlockGroupInfo, ctx: &CoordinatorContext) -> Option<BGOperator> {
        ...
        let table = ctx.bgtable_manager.get_table(bg.table_id)?;
        let desired = table.replica_count() as usize;
        let available_replicas = repair_available_replicas(bg, ctx);

        if available_replicas.len() < desired {
            return Self::build_under_replicated_repair(bg, &available_replicas, ctx);
        }

        if bg.replica_set.len() > desired {
            return Self::build_over_replicated_repair(bg, &bg.replica_set, ctx);
        }
...
mod tests {
    ...
    use crate::pd::coordinator::OpPriority;
    use curvine_common::state::ReplicaState;
    use curvine_common::state::StorageType;

    #[test]
    fn name_and_priority() {
        let checker = HashReplicaChecker;
    ...
    fn skips_bg_without_table() {
        let f = Fixture::new();
        let bg = f.insert_bg(1, 1, vec![100], None);
        // No table inserted → check_bg returns None (table lookup fails via `?`).
        assert!(HashReplicaChecker.check_bg(&bg, &f.ctx).is_none());
    }
    ...
    fn mark_unavailable(f: &Fixture, wids: &[u32]) {
        for &worker_id in wids {
            if f.ctx.node_manager.get_node(worker_id).is_some() {
                f.set_worker_state(worker_id, NodeState::Blacklist);
            }
        }
    }
    ...
    fn cases() -> Vec<Case> {
        vec![
            Case {
                ...
                unavailable_wids: vec![],
                candidate_workers: Some(vec![100, 101, 102, 103]),
                expect: Expect::None,
                expect_priority: None,
            },
            Case {
                name: "one replica unavailable -> add 1",
                desired_replicas: 3,
                replica_set: vec![100, 101, 102],
                unavailable_wids: vec![102],
            ...
            Case {
                name: "two replicas unavailable -> add 2",
                desired_replicas: 3,
                replica_set: vec![100, 101, 102],
                unavailable_wids: vec![101, 102],
            ...
            Case {
                name: "unavailable extra replica -> remove 1",
                desired_replicas: 3,
                replica_set: vec![100, 101, 102, 103],
                unavailable_wids: vec![103],
                candidate_workers: Some(vec![100, 101, 102, 103]),
                expect: Expect::RemoveReplicas(1),
                expect_priority: Some(OpPriority::OVER_REPLICA_REPAIR),
            },
    ...
    fn table_driven_check_bg() {
        for case in cases() {
            ...
            if let Some(ref workers) = case.candidate_workers {
                f.add_workers(workers, StorageType::Ssd);
            }
            let table_id = f.insert_table(StorageType::Ssd, case.desired_replicas);
            let bg = f.insert_bg(1, table_id, case.replica_set.clone(), None);
            mark_unavailable(&f, &case.unavailable_wids);

            let op = HashReplicaChecker.check_bg(&bg, &f.ctx);
            match case.expect {
                Expect::None => {
                ...
                Expect::AddReplicas(n) => {
                    ...
                    let wait_active = op
                        ...
                        .filter(|step| {
                            matches!(
                                step,
                                crate::pd::coordinator::OpStep::WaitReplicaReady {
                ...
                Expect::RemoveReplicas(n) => {
                    ...
                    let non_primary = bg.replica_set.iter().filter(|&&w| w != primary).count();
                    if n <= non_primary {
                        assert!(
                            !removed.contains(&primary),
                            "{}: must not remove primary when non-primary replicas suffice",
                            case.name,
                        );
                    }
                    if let Some(expected_prio) = case.expect_priority {
    ...
    #[test]
    fn over_replicated_removes_primary_only_as_last_resort() {
        // 5 replicas, desired=3 → remove 2. Primary = 100. Should remove two non-primary workers.
        let f = Fixture::new();
        f.add_workers(&[100, 101, 102, 103, 104], StorageType::Ssd);
        let table_id = f.insert_table(StorageType::Ssd, 3);
        let bg = f.insert_bg(1, table_id, vec![100, 101, 102, 103, 104], Some(100));

        let op = HashReplicaChecker.check_bg(&bg, &f.ctx).expect("op");
        let (_, removed, _) = decompose(&op);
        assert_eq!(removed.len(), 2);
        assert!(!removed.contains(&100), "primary 100 removed too eagerly");
    }

    #[test]
    fn over_replicated_can_fall_back_to_primary_when_needed() {
        // desired=1, replica_set=[100, 101] with primary=100. Remove 1 non-primary (101).
        let f = Fixture::new();
        f.add_workers(&[100, 101], StorageType::Ssd);
        let table_id = f.insert_table(StorageType::Ssd, 1);
    ...
    fn over_replica_lost_replicas_treated_as_healthy() {
        ...
        f.set_worker_state(102, NodeState::Lost);

        let op = HashReplicaChecker.check_bg(&bg, &f.ctx).expect("op");
        let (_, removed, _) = decompose(&op);
        assert_eq!(removed.len(), 1);
        // Whichever non-primary replica policy picks is fine; primary 100 must
        // not be removed.
    ...
    fn skips_bg_with_replica_on_leaving_node() {
        ...
        for leaving_state in [
            ...
            assert!(
                HashReplicaChecker.check_bg(&bg, &f.ctx).is_none(),
    ...
    fn takes_over_after_grace_window_expires() {
        ...
        // Without grace, checker takes over: replica_set=4, desired=3 → remove 1.
    ...
    #[test]
    fn over_replica_transfers_primary_when_owner_is_removed() {
        // 5 replicas, desired=3 → remove 2. Primary = 102 (which is on a
        // Decommission node, so cascade picks it for removal). Operator must
        // include a TransferPrimary step to a survivor before the removes.
        let mut overrides = std::collections::HashMap::new();
        overrides.insert(
            keys::PD_CHECKER_LEAVING_GRACE_MS.to_string(),
            "0".to_string(),
        ...
        assert_eq!(transfers.len(), 1, "primary removed → transfer expected");
        let (from, to) = transfers[0];
        assert_eq!(from, 102, "transfer must originate from removed primary");
        assert!(!removed.contains(&to), "transfer target must be a survivor");
    }

    #[test]
    fn over_replica_no_transfer_when_owner_survives() {
        // Sanity: primary stays in the surviving set → no TransferPrimary.
        let mut overrides = std::collections::HashMap::new();
        overrides.insert(
            keys::PD_CHECKER_LEAVING_GRACE_MS.to_string(),
            "0".to_string(),
    ...
    #[test]
    fn over_replica_keeps_primary_when_no_survivor_available() {
        let mut overrides = std::collections::HashMap::new();
        overrides.insert(
            keys::PD_CHECKER_LEAVING_GRACE_MS.to_string(),
            "0".to_string(),
        ...
        let f = Fixture::with_overrides(overrides);
        f.add_workers(&[100, 101], StorageType::Ssd);
        let table_id = f.insert_table(StorageType::Ssd, 0);
        let bg = f.insert_bg(1, table_id, vec![100, 101], Some(100));
        ...
        assert!(
            ...
            "primary must be kept when no survivor exists; got removed={:?}",
            removed,
        );
        assert!(
            transfers.is_empty(),
