...
use crate::pd::coordinator::{BGOperator, OpPriority, OperatorBuilder, OperatorKind};
use curvine_common::state::{BGKind, BlockGroupInfo, ReplicaState};

pub struct HashPlacementRuleChecker;

impl super::Checker for HashPlacementRuleChecker {
    fn name(&self) -> &str {
    ...
    fn check_bg(&self, bg: &BlockGroupInfo, ctx: &CoordinatorContext) -> Option<BGOperator> {
        ...
        let all_worker_ids: Vec<u32> = ctx.pool_manager.get_live_workers(table.storage_type());
        ...
        if bg.primary.node_id == worst_worker {
            let to_worker = ctx.pick_primary_fallback(bg, worst_worker, new_worker);
            builder = builder.transfer_primary(worst_worker, to_worker);
        }
...
mod tests {
    ...
    fn name_and_priority() {
        let checker = HashPlacementRuleChecker;
    ...
    fn no_op_when_no_min_isolation_level() {
        ...
        f.add_worker(100, StorageType::Ssd, &[("az", "a")]);
        f.add_worker(101, StorageType::Ssd, &[("az", "a")]);
        f.add_worker(102, StorageType::Ssd, &[("az", "b")]);
        let table_id = f.insert_table(StorageType::Ssd, 3);
        let bg = f.insert_bg(1, table_id, vec![100, 101, 102], None);
        assert!(HashPlacementRuleChecker.check_bg(&bg, &f.ctx).is_none());
    }
        ...
        vec![
            ...
            Case {
                ...
                expect: Expect::Fix {
                    ...
                    expect_primary_transfer: true, // primary 100 is one of the worst candidates
                },
            },
            Case {
                name: "shared AZ but worst is not primary → no TransferPrimary",
                replica_set: vec![100, 101, 200],
                primary: 200, // different AZ than 100/101
                workers: vec![
                    (100, &[("az", "a")]),
                    (101, &[("az", "a")]),
                    (200, &[("az", "b")]),
    ...
    fn table_driven_check_bg() {
        for case in cases() {
            ...
            for (wid, labels) in &case.workers {
                f.add_worker(*wid, StorageType::Ssd, labels);
            }
            let table_id = f.insert_table(StorageType::Ssd, 3);
            let bg = f.insert_bg(1, table_id, case.replica_set.clone(), Some(case.primary));

            let op = HashPlacementRuleChecker.check_bg(&bg, &f.ctx);
            match case.expect {
                Expect::None => {
                ...
                Expect::Fix {
                    ...
                    if expect_primary_transfer {
                        assert_eq!(
                            transfer.len(),
                            1,
                            "{}: expected primary transfer",
                            case.name
                        );
                        assert_eq!(
                        ...
                        assert!(transfer.is_empty(), "{}: no primary transfer", case.name);
                    }
                }
            }
        }
