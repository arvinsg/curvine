...
impl super::Checker for PoolMembershipChecker {
    ...
    fn check_bg(&self, _bg: &BlockGroupInfo, _ctx: &CoordinatorContext) -> Option<BGOperator> {
        None
    }

    fn check_worker(&self, worker: &NodeInfo, ctx: &CoordinatorContext) -> Option<BGOperator> {
        ...
        {
            Ok(changed_pools) if changed_pools.is_empty() => {
                log::warn!(
                    "PoolMembershipChecker: worker {} still has no pool membership change",
                    wid
                );
            }
            Ok(changed_pools) => {
                log::info!(
                    "PoolMembershipChecker repaired worker {}: changed_pools={:?}",
                    wid,
                    changed_pools
                );
            }
...
mod tests {
    ...
    fn live_worker_in_pool_is_noop() {
        let f = Fixture::new();
        f.add_workers(&[100], StorageType::Ssd);
