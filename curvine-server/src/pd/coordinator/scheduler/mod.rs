...
pub mod hash_bg_table_rebuild;
pub mod hash_decommission;
pub mod hash_primary_balance;
pub mod hash_replica_balance;
pub mod stats;

use crate::pd::bgtable::placement::context::PendingInfluence;
use crate::pd::coordinator::{BGOperator, CoordinatorContext, OperatorController};
...
pub trait Scheduler: Send + Sync {
    ...
    fn on_event(&self, _event: &CoordinatorEvent) {}
...
pub fn build_pending_influence(oc: &OperatorController) -> PendingInfluence {
    let (bg_delta, primary_delta) = oc.get_all_pending_deltas();
    PendingInfluence {
        bg_delta,
        primary_delta,
    }
...
pub fn default_schedulers(ctx: std::sync::Arc<CoordinatorContext>) -> Vec<Box<dyn Scheduler>> {
    vec![
        Box::new(hash_replica_balance::HashReplicaBalanceScheduler::default()),
        Box::new(hash_primary_balance::HashPrimaryBalanceScheduler::default()),
        Box::new(hash_bg_table_rebuild::HashBGTableRebuildScheduler::new(
            ctx.clone(),
        )),
        Box::new(hash_decommission::HashDecommissionScheduler::new()),
