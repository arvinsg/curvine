...
use curvine_common::state::{BGKind, BGStats, NodePayload, NodeType};
...
impl Scheduler for StatsScheduler {
    ...
    fn schedule(&self, ctx: &CoordinatorContext) -> Vec<BGOperator> {
        ...
        for node in ctx.node_manager.get_nodes_by_type(NodeType::Worker) {
            ...
            let stats_map: HashMap<curvine_common::state::BgId, BGStats> = payload
                .bg_reports
                .iter()
                .map(|r| (r.bg_id, r.stats.clone()))
                .collect();
            ctx.bgtable_manager.bg().update_bg_stats(BGKind::Hash, &stats_map);
        }

        // Aggregate BG-level stats into each BGTable.stats.
        ctx.bgtable_manager.refresh_table_stats();
...
mod tests {
    ...
    fn schedule_aggregates_bg_reports_into_table_stats() {
        ...
        let before = f.ctx.bgtable_manager.get_table_stats(table_id);
        ...
        let stats = f.ctx.bgtable_manager.get_table_stats(table_id);
    ...
    fn multiple_schedule_calls_refresh_table_stats() {
        ...
        s.schedule(&f.ctx);
        assert_eq!(
            f.ctx.bgtable_manager.get_table_stats(table_id).used_bytes,
            100
        );
