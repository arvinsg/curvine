...
use crate::pd::bgtable::placement::{
    create_hash_policy, is_primary_gap_sufficient, HashPlacementContext, PlacementContext,
};
...
#[derive(Default)]
pub struct HashPrimaryBalanceScheduler {
    last_register_ms: AtomicU64,
}

impl Scheduler for HashPrimaryBalanceScheduler {
    fn name(&self) -> &str {
        "hash-primary-balance-scheduler"
    }
    ...
    fn schedule(&self, ctx: &CoordinatorContext) -> Vec<BGOperator> {
        ...
        for table in &tables {
            ...
            // Find source workers (primary overloaded).
            let source_workers: Vec<u32> = placement_ctx
                .worker_ids()
                .into_iter()
                .filter(|&wid| balance_policy.is_primary_overloaded(&placement_ctx, &st, wid))
                .collect();
            ...
            for &source_id in &source_workers {
                ...
                for bg in &source_bgs {
                    ...
                    let replica_targets: Vec<u32> = bg
                        ...
                        .filter(|&w| {
                            ctx.bgtable_manager.bg().get_replica_state(BGKind::Hash, bg.bg_id, w)
                                == ReplicaState::Active
                        })
                        .collect();

                    let target = match balance_policy.select_primary(&st, &replica_targets) {
                        Ok(t) => t,
                        Err(_) => continue,
                    };

                    if !is_primary_gap_sufficient(&st, source_id, target, tolerant_ratio) {
                        continue;
                    }
                    ...
                    let builder = OperatorBuilder::new(
                        ...
                        bg.bg_id,
                        format!(
                            "Primary balance: transfer from worker {} to {}",
                            source_id, target
                        ),
                    )
                    .bg_epoch(bg.bg_epoch)
                    .priority(OpPriority::PRIMARY_BALANCE)
                    .transfer_primary(source_id, target);

                    result.push(builder.build());

                    st.record_primary_change(Some(source_id), target);
                    table_ops += 1;
                    break; // One primary per source per cycle.
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn name_and_type() {
        let s = HashPrimaryBalanceScheduler::default();
        assert_eq!(s.name(), "hash-primary-balance-scheduler");
    }
    ...
    /// Seed `bg_count` BGs with the same replica_set where worker `owner` is the primary holder.
    ...
    fn seed_bgs_with_owner(
        ...
        let bg_ids: Vec<curvine_common::state::BgId> =
            (0..bg_count).map(|i| 2_000 + i as u64).collect();
        for &bg_id in &bg_ids {
    ...
    fn disabled_by_config() {
        ...
        assert!(!HashPrimaryBalanceScheduler::default().is_schedule_allowed(&f.ctx));
    }
    ...
    fn no_ops_when_fewer_than_two_workers() {
        ...
        assert!(HashPrimaryBalanceScheduler::default()
            .schedule(&f.ctx)
            .is_empty());
    }

    #[test]
    fn no_ops_when_primary_balanced() {
        // Each worker is primary for exactly 2 BGs (8 BGs / 4 workers = 2).
        let f = Fixture::new();
        ...
        let ops = HashPrimaryBalanceScheduler::default().schedule(&f.ctx);
        assert!(
            ops.is_empty(),
            "balanced primaries should yield no ops, got {:?}",
            ops
        );
    }

    #[test]
    fn overloaded_primary_triggers_transfer() {
        // Worker 100 owns ALL 8 primaries; other replicas in replica_set get zero.
        let f = Fixture::new();
        f.add_workers(&[100, 101, 102, 103], StorageType::Ssd);
        let table_id = f.insert_table(StorageType::Ssd, 3);
        ...
        assert!(
            !ops.is_empty(),
            "overloaded primary should yield at least one transfer"
        );

        for op in &ops {
            assert_eq!(op.priority, OpPriority::PRIMARY_BALANCE);
            let (add, remove, transfer) = decompose(op);
            assert!(
                add.is_empty() && remove.is_empty(),
                "primary balance uses TransferPrimary only"
            );
            assert_eq!(transfer.len(), 1, "one TransferPrimary");
            let (from, to) = transfer[0];
            assert_eq!(from, 100, "source is overloaded primary");
            assert!(
                [101, 102].contains(&to),
    ...
    #[test]
    fn skips_bg_when_source_is_not_primary() {
        let f = Fixture::new();
        f.add_workers(&[100, 101, 102, 103], StorageType::Ssd);
        let table_id = f.insert_table(StorageType::Ssd, 3);
        ...
        for op in &ops {
            ...
            assert_eq!(
                transfer[0].0, 103,
                "source must be the overloaded primary holder"
            );
        }
    }
    ...
    fn skips_non_idle_bgs() {
        ...
        for &bg_id in &bg_ids {
            f.ctx.bgtable_manager.bg().set_op_state(
                BGKind::Hash,
                bg_id,
                curvine_common::state::BGOpState::Repairing,
            );
        }

        let ops = HashPrimaryBalanceScheduler::default().schedule(&f.ctx);
        assert!(ops.is_empty(), "all BGs non-Idle → no primary balance ops");
    }
}
